...
const myPeer = new Peer({
  port: '443'
}) // previous parameters were causing connectivity issues
let myVideoStream;
...
navigator.mediaDevices.getUserMedia({
  ...
  socket.on('addparticipant', userName => {
    pset.add(`<li class="message" ><b>${userName}</b></li>`)
    const myNode1 = document.getElementById("p");
    while (myNode1 && myNode1.firstChild) {
      myNode1.removeChild(myNode1.lastChild);
    }
    for (let elements of pset)
    {
      if(elements)
      $('#p').append(elements)
      else set.delete(elements)
    }
  })
  // input value
  let text = $("input");
